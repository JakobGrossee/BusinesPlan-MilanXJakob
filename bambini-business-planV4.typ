...
Die wirtschaftliche Planung versteht *das erste Geschäftsjahr als Validierungs- und Aufbaujahr*. Umsätze entstehen erst ab Juli 2026 durch die kostenpflichtige Zusatzleistung. Profitabilität ist geplant ab Skalierung der Zusatzleistungen, weiterer Antragsarten (Kindergeld, Elternzeit ab 2027) und möglicher weiterer Erlöse. // hier ggf. noch konkretisieren
...
Der Markt ist nicht gänzlich unerschlossen, aber fragmentiert. Drei Anbietergruppen prägen das Bild:
...
#grid(
  columns: (1fr, 1fr, 1fr),
  column-gutter: 10pt,
  box(fill: light-bg, inset: 12pt, radius: 8pt, width: 100%, stroke: (top: 3pt + info, rest: 1pt + surface))[
    #text(weight: "bold", fill: info, size: 10pt)[Staatliche Angebote]
    #v(4pt)
    #text(size: 8.5pt)[Behördliche Formulare und Informationen - kostenlos, jedoch komplex, fragmentiert nach Bundesländern und ohne durchgängige Prozessführung.]
  ],
  box(fill: light-bg, inset: 12pt, radius: 8pt, width: 100%, stroke: (top: 3pt + accent, rest: 1pt + surface))[
    #text(weight: "bold", fill: accent.darken(10%), size: 10pt)[Private Self-Service-Anbieter]
    #v(4pt)
    #text(size: 8.5pt)[Typischerweise *#competitor-price-low - #competitor-price-high €* für Antragsvorbereitung. Fokus meist auf einzelne Leistung (z. B. nur Elterngeld). Ausdrucken erfolgt durch Nutzer selbst.]
  ],
  box(fill: light-bg, inset: 12pt, radius: 8pt, width: 100%, stroke: (top: 3pt + primary, rest: 1pt + surface))[
    #text(weight: "bold", fill: primary, size: 10pt)[Lücke für Bambini]
    #v(4pt)
    #text(size: 8.5pt)[Bisher fehlt die Kombination aus *kostenlosem Einstieg*, Zusammenfassung aller gewöhnlichen Anträge und perspektivisch Antragsabwicklung in einer Hand.] //diese Formulierung noch einmal überarbeiten 
    // hier ggf auch bereits die wettbewerbertabelle einfügen?
  ],
)
...
#highlight-box(color: info, title: [BambiniAI - KI-Assistent als Differenzierung])[
  ...
  *Die KI-Nutzung ist kein wesentlicher Kostentreiber:* Pro Freitextanfrage fallen lediglich rund *#euro(ai-cost-per-request)* an. Selbst bei intensiver Nutzung bleiben die KI-Kosten in einer Größenordnung, die das Geschäftsmodell auch im kostenfreien Einstieg tragähig lässt.
]
#pagebreak()
...
#highlight-box(color: info, title: [Einordnung der Wirtschaftlichkeit])[
  Der Preis von #euro(premium-price) eignet sich für eine niedrigschwellige Zusatzleistung, trägt allein aber kein starkes Gewinnmodell. Für eine höhere langfristige Wirtschaftlichkeit sind erforderlich: (a) deutlich höhere Nutzerzahlen, (b) zusätzliche Antragsarten (Kindergeld, Elternzeit ab Y2), (c) höherwertige Servicepakete und (d) Partner-/B2B-Erlöse. Diese Erlöse werden in der Basisplanung *bewusst nicht als garantiert* angesetzt.
]
...
#grid(
  ...
  box(
    ...
    #v(8pt)
    #text(size: 8.5pt)[
      Bambini ist *technisches Hilfsmittel*, keine Rechtsberatung im Sinne des § 2 Abs. 1 RDG. Berechnungen sind rein rechnerisch, ohne individuelle rechtliche Würdigung. Nutzer bestätigen ihre Angaben vor Versand und behalten die Eigenverantwortung. BambiniAI gibt nur allgemeine Orientierung. Haftung ist in den AGB auf Vorsatz und grobe Fahrlässigkeit beschränkt (§ 309 Nr. 7 BGB-konform). Der Premium-Prozess bleibt bewusst standardisiert; eine Ausweitung der Service-Tiefe erfolgt erst nach erneuter rechtlicher Prüfung.
    ]
  ],
  ...
  box(
    ...
    #v(8pt)
    #text(size: 8.5pt)[
      Kerndaten verbleiben im EWR (Supabase Frankfurt, Living-Bots Bremen). Datenminimierung, klare Zweckbindung und Löschkonzept sind Teil des Produkts. KI-Anfragen laufen über OpenRouter mit Zero Data Retention. Ein externer Datenschutzbeauftragter ist bei < 20 MA gesetzlich nicht erforderlich (§ 38 BDSG). Datenschutzerklärung und AGB sind erstellt und veröffentlicht.
    ]
  ],
  ...
  box(
    ...
    #v(8pt)
    #text(size: 8.5pt)[
      Pro Freitextanfrage fallen lediglich rund *#euro(ai-cost-per-request)* an - KI-Nutzung ist damit *kein wesentlicher finanzieller Risikofaktor*. Zusätzlich greift FAQ-first (Standardfragen ohne Modellaufruf), Nutzungslimits, Tages- und Nutzerbudgets sowie eine Anbieter-/Modell-Whitelist via OpenRouter. Anomalie-Monitoring sichert das Modell zusätzlich gegen Missbrauch ab.
    ]
  ],
)
...
#highlight-box(color: success, title: [Was Bambini ist - und was Bambini bewusst nicht ist])[
  Bambini startet *fokussiert und konservativ*: kostenloser Elterngeld-Self-Service, als höherwertige Alternative zu Wettbewerbern. Im Anschluss findet eine monetarisierbare, kontrollierte Skallierung statt.
